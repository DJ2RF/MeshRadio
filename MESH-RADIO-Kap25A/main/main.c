// ============================================================================
// MeshRadio – Kapitel 25A (RSSI-Aware Routing + Hysterese) – KOMPLETTE SOURCEN
// ----------------------------------------------------------------------------
// Baut auf Kapitel 25 auf (Destination Routing) und erweitert um:
//
// Kapitel 25A Features:
//   - Qualitätsbasierte Routenwahl (RSSI-aware)
//   - Hysterese gegen Route-Flapping (z.B. 5 dB)
//   - Optional: Quality Routing ON/OFF zur Gegenprobe im Labor
//   - Web UI + API:
//       * /api/send        (dst,msg)
//       * /api/routes      (Tabelle dst->next inkl. rssi/age)
//       * /api/bcfallback  (enable=0/1)
//       * /api/qroute      (enable=0/1)   // RSSI-Quality Routing an/aus
//       * /api/hyst        (db=0..20)     // Hysterese in dB
//
// Hinweis:
//   - Beacons bleiben die Lernquelle: dst=src -> next=last_hop, rssi=Rx-RSSI
//   - route_update_locked() entscheidet jetzt, ob ein neuer Eintrag "gut genug"
//     ist, um den alten zu ersetzen (RSSI + Hysterese).
// ============================================================================
...
// Kapitel 25A: Hysterese (dB) gegen "Flapping"
// - nur wenn new_rssi > old_rssi + hyst wird die Route ersetzt
#define DEFAULT_ROUTE_HYST_DB 5
...
// Kapitel 25A: Quality Routing + Hysterese
// - g_quality_routing: wenn false, Verhalten wie Kapitel 25 (update wenn besser ohne Schwelle)
// - g_route_hyst_db  : Schwelle in dB
static bool g_quality_routing=true;
static int  g_route_hyst_db=DEFAULT_ROUTE_HYST_DB;
...
// ============================================================================
// ROUTES (Destination-based) – Kapitel 25A Erweiterung
// ----------------------------------------------------------------------------
// Idee:
//   - Route wird nicht bei jeder kleinen RSSI-Schwankung ersetzt.
//   - Mit Hysterese wird nur dann gewechselt, wenn der neue Pfad deutlich
//     besser ist (new_rssi > old_rssi + hyst_db).
//
// Zusätzlich:
//   - Timeout/Aging bleibt: alte Einträge können ersetzt/gelöscht werden.
// ============================================================================
static bool rssi_is_better_with_hyst(int new_rssi, int old_rssi, int hyst_db)
{
    // Achtung: RSSI ist negativ (z.B. -70 > -90 ist "besser").
    // Beispiel: old=-82, hyst=5 => new muss >= -77 sein.
    return (new_rssi > (old_rssi + hyst_db));
}
...
static void route_update_locked(const char dst[7], const char next[7], int rssi)
{
    ...
    for(int i=0;i<MAX_ROUTES;i++){
        ...
        // "Better"-Definition hängt von Kapitel 25A Schalter ab:
        // - Quality ON  => nur ersetzen, wenn new deutlich besser (Hysterese)
        // - Quality OFF => ersetzen, wenn new einfach besser als old
        bool better = false;
        if(g_quality_routing){
            better = rssi_is_better_with_hyst(rssi, routes[i].rssi, g_route_hyst_db);
        }else{
            better = (rssi > routes[i].rssi);
        }

        // Ersetzen wenn:
        // - Eintrag abgelaufen ist, oder
        // - der neue Pfad signifikant besser ist
        if(expired || better){
            memcpy(routes[i].next,next,7);
            routes[i].rssi=rssi;
        }

        // Timestamp immer aktualisieren: Route "lebt", solange Beacons rein kommen
        routes[i].t_ms=t;
        return;
    }

    // Falls dst noch nicht existiert: neu anlegen
    for(int i=0;i<MAX_ROUTES;i++){
        if(!routes[i].used){
            routes[i].used=true;
    ...
    // Tabelle voll: ältesten Eintrag überschreiben
    int oldest=0;
...
static void handle_rx(void)
{
    ...
    if(h->flags & MR_FLAG_BEACON){
        ...
        // Kapitel 25A: Route Learning bleibt Beacon-basiert
        // - dst = beacon.src
        // - next = beacon.last_hop
        // - rssi = Empfangsqualität dieses Links
        xSemaphoreTake(g_mutex,portMAX_DELAY);
...
// FIXED (Kapitel 25): restores '=' also on match
...
"<title>MeshRadio Kapitel 25A</title>"
"<style>"
...
"</style></head><body>"
"<h2>MeshRadio Kapitel 25A – RSSI-Aware Routing + Hysterese</h2>"
...
"<h3>Quality Routing</h3>"
"<button onclick='setQR(1)'>Quality ON</button>"
"<button onclick='setQR(0)'>Quality OFF</button>"
"<label>Hysterese (dB)</label><input id='h' value='5'>"
"<button onclick='setH()'>Set Hysterese</button>"
...
"async function setQR(v){ await post('/api/qroute','enable='+encodeURIComponent(v)); }"
"async function setH(){"
" let db=document.getElementById('h').value;"
" await post('/api/hyst','db='+encodeURIComponent(db));"
"}"
...
static esp_err_t api_routes_get(httpd_req_t *req)
{
    ...
    off += snprintf(out+off,sizeof(out)-off,
                    ...
                    "bc_fallback=%u  quality=%u  hyst_db=%d\n\n",
                    g_bc_fallback?1:0, g_quality_routing?1:0, g_route_hyst_db);
...
// Kapitel 25A: Quality Routing ON/OFF
static esp_err_t api_qroute_post(httpd_req_t *req)
{
    char body[64];
    if(!http_read_body(req,body,sizeof(body))) return http_send_text(req,"ERR body");
    ...
    xSemaphoreTake(g_mutex,portMAX_DELAY);
    g_quality_routing = en;
    xSemaphoreGive(g_mutex);

    return http_send_text(req, en ? "OK quality=1" : "OK quality=0");
}

// Kapitel 25A: Hysterese setzen
static esp_err_t api_hyst_post(httpd_req_t *req)
{
    char body[64];
    if(!http_read_body(req,body,sizeof(body))) return http_send_text(req,"ERR body");

    char v[16]={0};
    if(!form_get(body,"db",v,sizeof(v))) return http_send_text(req,"ERR missing db");
    ...
    char out[64];
    snprintf(out,sizeof(out),"OK hyst_db=%d", db);
    return http_send_text(req,out);
}
...
static void http_start(void)
{
    ...
    httpd_uri_t q0={.uri="/api/qroute",.method=HTTP_POST,.handler=api_qroute_post};
    httpd_register_uri_handler(g_http,&q0);

    httpd_uri_t h0={.uri="/api/hyst",.method=HTTP_POST,.handler=api_hyst_post};
    httpd_register_uri_handler(g_http,&h0);
...
void app_main(void)
{
    ESP_LOGI(TAG,"MeshRadio Kapitel 25A start (RSSI-Aware Routing + Hysterese)");
    ...
    ESP_LOGI(TAG,"Quality     = %u", g_quality_routing?1:0);
    ESP_LOGI(TAG,"Hyst (dB)   = %d", g_route_hyst_db);
