// ============================================================================
// MeshRadio – Kapitel 22 (Sourcen komplett, sauber dokumentiert)
// ----------------------------------------------------------------------------
// CLI/UART KONFIGURATION + NVS PERSISTENZ
//
// Basierend auf Kapitel 21:
//
//   ✔ NVS Persistenz (call, freq, sf, bw, crc)
//   ✔ Beacon TX + Beacon RX (Route Refresh)
//   ✔ Header V3 (Kap. 19A: last_hop)
//
// NEU in Kapitel 22:
//
//   ✔ einfache CLI über UART/USB (115200)
//   ✔ Kommandos: help, show, call, freq, sf, bw, crc, apply, save, load, reboot,
//                beacon, beaconint
//   ✔ Validierung der Parameter
//   ✔ Config wird sofort geändert, aktiv wird sie durch "apply"
//   ✔ "save" schreibt in NVS (bleibt nach Reset)
//   ✔ "load" lädt aus NVS und macht apply
//
// Terminal:
//   - PuTTY / Screen / IDF Monitor
//   - 115200 baud
...
// Default Beacon Scheduling (kann per CLI geändert werden)
...
// ============================================================================
// Kapitel 21 Config (persistiert in NVS)
// ============================================================================
typedef struct {
...
static void call7_to_cstr(char out8[8], const char in7[7])
{
    memcpy(out8,in7,7);
    out8[7]=0;
    // Trim trailing spaces for nicer printing
    for(int i=6;i>=0;i--){
        if(out8[i]==' ') out8[i]=0;
        else break;
    }
...
// ============================================================================
// Routing + Neighbor cleanup (wie Kap 21)
// ============================================================================
...
// ----------------------------------------------------------------------------
// apply: setzt LoRa Hardware auf Basis der aktuellen Config
// Für Kapitel 22 halten wir es simpel:
//  - Frequency wird korrekt gesetzt
//  - Modem Config bleibt (noch) Default (wie bisher)
//  -> SF/BW/CRC werden im nächsten Kapitel sauber in die Register gemappt
// ----------------------------------------------------------------------------
static void lora_apply_config(const mr_config_t *cfg)
{
    ESP_LOGI(TAG,"APPLY: freq=%" PRIu32 " sf=%u bw=%u crc=%u call=%s",
             cfg->freq_hz, cfg->sf, cfg->bw_khz, cfg->crc_on, cfg->call);
...
static void beacon_schedule_next(void)
{
    next_beacon_ms = now_ms()
                   ...
                   + (esp_random() % (g_beacon_jitter_ms ? g_beacon_jitter_ms : 1));
}
...
static void send_beacon(void)
{
    ...
    ESP_LOGI(TAG,"TX BEACON id=%u call=%s freq=%" PRIu32, b.msg_id, g_cfg.call, g_cfg.freq_hz);
...
// CLI (Kapitel 22)
// ----------------------------------------------------------------------------
// Wir lesen Zeichen über getchar() (ESP-IDF Monitor / PuTTY / UART)
// - unterstützt LF oder CRLF
// - maximal 127 Zeichen
// ----------------------------------------------------------------------------
static void cli_print_help(void)
{
    printf(
        "\nMeshRadio CLI (Kapitel 22)\n"
        "----------------------------------------\n"
        "help                 - diese Hilfe\n"
...
static void cli_execute(char *line)
{
    ...
    if(strcmp(cmd,"call")==0){
        if(cli_set_call(arg)){
            printf("OK: call=%s (apply + save optional)\n", g_cfg.call);
        }else{
            printf("ERR: usage: call <CALL> (max 7)\n");
        }
        return;
    }
    ...
    if(strcmp(cmd,"freq")==0){
        ...
        if(cli_set_u32(arg,&f) && f>100000000 && f<1000000000){
            ...
            printf("ERR: usage: freq <HZ> (z.B. 433775000)\n");
        }
        return;
    }
    ...
    if(strcmp(cmd,"bw")==0){
        ...
        if(cli_set_bw(arg,&v)){
            ...
            printf("ERR: usage: bw <125|250|500>\n");
        }
        return;
    }
    ...
    if(strcmp(cmd,"beaconint")==0){
        ...
        if(cli_set_u32(arg,&ms) && ms>=1000 && ms<=600000){
            ...
            printf("ERR: usage: beaconint <ms> (1000..600000)\n");
        }
        return;
    }
...
// Poll one char; build line; execute on newline
...
void app_main(void)
{
    ESP_LOGI(TAG,"Kapitel 22 start (CLI + NVS)");
