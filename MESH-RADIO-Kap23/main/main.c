// ============================================================================
// MeshRadio – Kapitel 23 (Sourcen komplett, sauber dokumentiert)
// ----------------------------------------------------------------------------
// Basis: Kapitel 22B+ (WLAN Web-Setup + Beacon Web + NVS + SX1276 Config)
//
// NEU in Kapitel 23: LIVE-ANSICHT von Nachbarn & Routen
//   ✔ GET /api/neighbors  -> JSON Liste der Nachbarn (Call, RSSI, Age_ms)
//   ✔ GET /api/routes     -> JSON Liste der Routen (Dest, NextHop, RSSI, Age_ms)
//   ✔ Web-UI erweitert um:
//        - Button "Refresh Neighbors"
//        - Button "Refresh Routes"
//        - Tabellenanzeige im Browser
//
// Ziel:
//   - Beim Aufbau eines Mesh siehst du sofort, ob Beacons reinkommen
//   - und wie die Route-Tabelle wächst.
//
// Nutzung:
//   1) Flashen
...
// Beacon runtime params (Web änderbar)
...
// Ein Mutex schützt ALLE shared states (cfg + neighbors + routes + next_beacon_ms)
...
// ============================================================================
// Neighbor + Route tables (Kapitel 23: diese Daten wollen wir live anzeigen)
// ============================================================================
...
// lora_apply_config_locked(): g_mutex MUSS vorher gelockt sein!
...
// ============================================================================
// Web UI (Kapitel 23: erweitert um Neighbors/Routes Tabellen)
// ============================================================================
...
"<h3>Neighbors</h3>"
"<div class='row'><button onclick='refreshNeighbors()'>Refresh Neighbors</button></div>"
"<div id='neighbors'></div>"

"<h3>Routes</h3>"
"<div class='row'><button onclick='refreshRoutes()'>Refresh Routes</button></div>"
"<div id='routes'></div>"
...
"async function doBeaconSet(){ let b='enabled='+encodeURIComponent(q('beacon').value); await post('/api/beacon', b); }"
"async function doBeaconInt(){ let b='ms='+encodeURIComponent(q('beaconint').value); await post('/api/beaconint', b); }"

"function table(headers, rows){"
" let h='<table><thead><tr>' + headers.map(x=>'<th>'+x+'</th>').join('') + '</tr></thead><tbody>';"
" if(rows.length===0){ h+='<tr><td colspan=\"'+headers.length+'\">(empty)</td></tr>'; }"
" else{ for(let r of rows){ h+='<tr>'+r.map(c=>'<td>'+c+'</td>').join('')+'</tr>'; } }"
" return h+'</tbody></table>';"
"}"

"async function refreshNeighbors(){"
" let r=await fetch('/api/neighbors'); let j=await r.json();"
" let rows=j.neighbors.map(n=>[n.call, n.rssi, n.age_ms]);"
" q('neighbors').innerHTML = table(['CALL','RSSI (dBm)','AGE (ms)'], rows);"
"}"

"async function refreshRoutes(){"
" let r=await fetch('/api/routes'); let j=await r.json();"
" let rows=j.routes.map(e=>[e.dest, e.next, e.rssi, e.age_ms]);"
" q('routes').innerHTML = table(['DEST','NEXT HOP','RSSI (dBm)','AGE (ms)'], rows);"
"}"

"doShow(); refreshNeighbors(); refreshRoutes();"
"</script></body></html>";
...
// ============================================================================
// API: /api/show (Config + Beacon status)
// ============================================================================
...
// ============================================================================
// API: /api/neighbors (Kapitel 23)
//   Ausgabe: { "neighbors": [ {call:"DL1XYZ", rssi:-86, age_ms:1234}, ... ] }
// ============================================================================
static esp_err_t http_api_neighbors(httpd_req_t *req)
{
    // Wir bauen JSON in einen ausreichend großen Puffer.
    // MAX_NEIGHBORS=20 -> das passt locker in 2048.
    char json[2048];
    size_t pos = 0;
    ...
    pos += snprintf(json+pos, sizeof(json)-pos, "{\"neighbors\":[");
    bool first = true;
    ...
    for(int i=0;i<MAX_NEIGHBORS;i++){
        ...
        uint32_t age = t - neighbors[i].last_seen_ms;

        if(!first) pos += snprintf(json+pos, sizeof(json)-pos, ",");
        first = false;

        pos += snprintf(json+pos, sizeof(json)-pos,
                        "{\"call\":\"%s\",\"rssi\":%d,\"age_ms\":%" PRIu32 "}",
                        call8, neighbors[i].rssi_dbm, age);

        if(pos > sizeof(json)-128) break; // safety
    }
...
// ============================================================================
// API: /api/routes (Kapitel 23)
//   Ausgabe: { "routes": [ {dest:"DB0XYZ", next:"DL1ABC", rssi:-90, age_ms:8000}, ... ] }
// ============================================================================
static esp_err_t http_api_routes(httpd_req_t *req)
{
    char json[2048];
    size_t pos = 0;
    ...
    pos += snprintf(json+pos, sizeof(json)-pos, "{\"routes\":[");
    bool first = true;
    ...
    for(int i=0;i<MAX_ROUTES;i++){
        ...
        char dst8[8], hop8[8];
        call7_to_cstr(dst8, routes[i].destination);
        call7_to_cstr(hop8, routes[i].next_hop);
        uint32_t age = t - routes[i].last_seen_ms;

        if(!first) pos += snprintf(json+pos, sizeof(json)-pos, ",");
        first = false;

        pos += snprintf(json+pos, sizeof(json)-pos,
                        "{\"dest\":\"%s\",\"next\":\"%s\",\"rssi\":%d,\"age_ms\":%" PRIu32 "}",
                        dst8, hop8, routes[i].rssi_dbm, age);
...
// ============================================================================
// API: /api/set (RAM only)  body: call=...&freq=...&sf=...&bw=...&crc=...
// ============================================================================
static esp_err_t http_api_set(httpd_req_t *req)
{
    ...
    if(has_freq){
        ...
        if(!parse_u32(v_freq,&f) || f<100000000 || f>1000000000){
            xSemaphoreGive(g_mutex); return http_send_text(req,"ERR: freq (Hz) invalid");
        }
        g_cfg.freq_hz = f;
    }
...
// ============================================================================
// API: /api/beacon   body: enabled=0|1
// ============================================================================
...
// ============================================================================
// API: /api/beaconint   body: ms=1000..600000
// ============================================================================
...
static void http_server_start(void)
{
    ...
    // Kapitel 23 endpoints
    httpd_uri_t uN = {.uri="/api/neighbors", .method=HTTP_GET, .handler=http_api_neighbors, .user_ctx=NULL};
    httpd_register_uri_handler(g_http, &uN);

    httpd_uri_t uR = {.uri="/api/routes", .method=HTTP_GET, .handler=http_api_routes, .user_ctx=NULL};
    httpd_register_uri_handler(g_http, &uR);
...
void app_main(void)
{
    ESP_LOGI(TAG,"MeshRadio Kapitel 23 start (Web-Setup + Neighbors/Routes)");
    ...
    while(1){
        ...
        // Tabellen-Aufräumen hier auch locked, damit API konsistent bleibt
