...
// https://nerdverlag.com// ============================================================================
// MeshRadio – Kapitel 34 (Node Rollen: RELAY / EDGE / SENSOR)  + Kap.29 Security
// ----------------------------------------------------------------------------
// Basis: Kapitel 29 (AES-CCM + Replay-Schutz) mit SX1276 Registerdriver
//
// Kapitel 34 Features (NEU):
//   - Node Rollen (profi-level stabil):
//       * RELAY  (Backbone): Beacon + RouteAdv + Forwarding
//       * EDGE   (Randnode): Beacon ja (damit man ihn routen kann), RouteAdv nein, Forwarding nein
//       * SENSOR (Minimal): Beacon nein, RouteAdv nein, Forwarding nein (DATA senden/empfangen möglich)
//   - Web UI erweitert:
//       * Node Role anzeigen + umstellen (POST /api/role mode=0/1/2)
//   - /api/status liefert node_mode
//
// WICHTIG:
//   - Protokoll bleibt identisch. Rollen ändern nur lokales Verhalten.
//   - SENSOR/EDGE ACKen weiterhin Pakete, die an sie geliefert werden (iam_dst + ACKREQ).
//   - SENSOR/EDGE forwarden NICHT (und senden keine ACKs für fremde Pakete).
//
// API (zusätzlich zu Kap.29):
//   - POST /api/role              (mode=0|1|2)
// ----------------------------------------------------------------------------
// Node Rollen Default: DEFAULT_NODE_MODE (unten).
// ============================================================================
...
static void lora_send(const uint8_t *d,size_t len)
{
...
// Kapitel 34: Nur RELAY/EDGE senden Beacons. SENSOR nicht.
// ============================================================================
...
static void send_ack(const mr_hdr_v7_t *rx)
{
    ...
    h.ttl=ACK_TTL;
    h.msg_id = rx->msg_id;
    h.seq = g_my_seq; // ACK trägt keine neue Seq in diesem Design
...
// Kapitel 34: Nur RELAY advertised Routes.
// ============================================================================
...
static void maybe_advertise_routes(void)
{
    ...
    if(g_node_mode != NODE_RELAY) return; // Kapitel 34: nur RELAY
...
// Kapitel 34: Nur RELAY forwardet.
// ============================================================================
static void forward_data(uint8_t *buf, size_t len)
{
    if(g_node_mode != NODE_RELAY) return; // Kapitel 34
...
static void handle_rx(void)
{
    ...
    // BEACON -> learn (auch EDGE/SENSOR darf lernen)
    ...
    // ROUTEADV -> integrate (auch EDGE/SENSOR darf integrieren, RouteAdv senden aber nur RELAY)
    ...
    if(h->flags & MR_FLAG_DATA){
        ...
        if(iam_dst){
            ...
            // Wichtig: auch EDGE/SENSOR acken delivered DATA (wenn ACKREQ)
        ...
        // Kapitel 34:
        // - EDGE/SENSOR forwarden NICHT.
        // - Also: nur RELAY darf hier forwarden (und nur dann ACK für forwarding-pflichtige Frames senden).
        if((iam_nexthop || nh_wild) && h->ttl > 1){
            if(g_node_mode == NODE_RELAY){
...
// ============================================================================
// Web UI – Kapitel 34 (Node Role Panel + Security Panel + Stats)
// ============================================================================
static const char *INDEX_HTML =
"<!doctype html><html><head><meta charset='utf-8'>"
"<meta name='viewport' content='width=device-width,initial-scale=1'>"
"<title>MeshRadio Kapitel 34</title>"
"<style>"
...
"</style></head><body>"
"<h2>MeshRadio Kapitel 34 – Node Rollen (profi-level stabil)</h2>"
...
"  <label>Text</label><input id='msg' value='Hello 34 (roles + secure)'>"
...
"  <div class='muted'>RELAY: Beacon+RouteAdv+Forward • EDGE: Beacon only • SENSOR: no Beacon/Forward</div>"
" </div>"
...
"    'call='+j.call+' • mode='+j.node_mode_str+' • crypto='+j.crypto_enable+' • routes='+j.routes.length+' • neighbors='+j.neighbors.length;"
...
// ============================================================================
// API: /api/status (JSON) – includes crypto + security stats + node_mode
// ============================================================================
...
static esp_err_t api_role_post(httpd_req_t *req)
{
    ...
    int v = atoi(m);
    if(v < 0 || v > 2) return http_send_text(req, "ERR mode must be 0(RELAY),1(EDGE),2(SENSOR)");
...
static void http_start(void)
{
    ...
    httpd_uri_t r0={.uri="/api/role",.method=HTTP_POST,.handler=api_role_post};
...
void app_main(void)
{
    ESP_LOGI(TAG,"MeshRadio Kapitel 34 start (Node Rollen + AES-CCM + Replay)");
    ...
    ESP_LOGI(TAG,"CALL=%s  mode=%s  crypto=%u  net_id=0x%02X",
             MR_CALLSIGN, node_mode_str(g_node_mode), g_crypto_enable?1:0, (unsigned)MR_NET_ID);
    ESP_LOGI(TAG,"Open http://192.168.4.1");

    while(1){
        vTaskDelay(pdMS_TO_TICKS(50));

        // Kapitel 34: Beacon nur RELAY/EDGE (Sensor nicht)
        if(g_beacon_enabled && g_node_mode != NODE_SENSOR && now_ms() > g_next_beacon_ms)
            send_beacon();

        // Kapitel 34: RouteAdv nur RELAY
