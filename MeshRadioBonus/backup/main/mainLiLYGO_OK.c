...
//       * SX1262         (Command-Driver)  -> Heltec LoRa 32 V3.2
//
// WICHTIG:
//   - Heltec V3.2 hat SX1262, nicht SX1276.
//   - GPIO-Konflikte werden abgefangen (z.B. Heltec: DIO1=14 -> RELAY_GPIO darf NICHT 14 sein)
//
// FIXES die wir hier konsolidiert behalten:
//   A) Web-UI "loading..." Fix: JSON immer null-terminiert + Bounds-Checks
//   B) UART/CLI hängt nicht mehr: LoRa TX hat Timeout + Recovery
//   C) SPI Assert Fix: SPI mutex für LoRa (für beide Chips)
//   D) handle_rx() unlock-safe: LoRa SPI nur im SPI-Lock, Tabellen nur im g_mutex
// ============================================================================
...
// ============================================================================
//
// Wir setzen hier _einheitlich_ folgende LoRa-Signale:
...
//
// Für ADC/Relay setzen wir ebenfalls Board-sichere Defaults.
// DU KANNST das unten natürlich an deine Verdrahtung anpassen.
//
// ============================================================================
#if (MR_BOARD_PRESET == MR_BOARD_HELTEC_V3)
// Heltec LoRa 32 V3.x (SX1262) – typische Pins
// Hinweis: Diese Zuordnung entspricht weit verbreiteten Heltec V3 Mappings.
...
// ============================================================================
//
// Wir kapseln SX1276 und SX1262 hinter denselben API-Funktionen:
//
//   lora_hw_reset()
//   lora_init_radio()
//   lora_set_rx_continuous()
//   lora_send_frame(buf,len)
//   lora_try_read_frame(buf,max,&len,&rssi)
//   lora_recover_radio()
//
// Damit bleibt dein gesamter Mesh/Proto Code gleich.
// ============================================================================
...
static void lora_init_radio(void)
{
    ...
    // 433.775 MHz / SF7 / BW125 / CR4/5 / CRC on
...
static void lora_init_radio(void)
{
    ...
    // Standby RC
    uint8_t stby[2] = { SX126X_SET_STANDBY, 0x00 };
    sx126x_cmd(stby, sizeof(stby));

    // Packet type: LoRa (0x01)
    uint8_t ptype[2] = { SX126X_SET_PACKET_TYPE, 0x01 };
    sx126x_cmd(ptype, sizeof(ptype));

    // RF frequency: 433.775 MHz
    // SX126x uses PLL steps: Freq = (rfFreq * 32MHz) / 2^25
    // rfFreq = (freq_hz << 25) / 32e6
    uint32_t freq = 433775000UL;
    uint32_t rf = (uint32_t)(((uint64_t)freq << 25) / 32000000ULL);
    ...
    // Modulation params:
    // SF7, BW125kHz, CR4/5, LDRO=0
    // BW enum (SX126x): 125kHz => 0x04 (common mapping)
    // CR 4/5 => 0x01
    uint8_t mod[5] = { SX126X_SET_MODULATION, 0x07, 0x04, 0x01, 0x00 };
    sx126x_cmd(mod, sizeof(mod));

    // Packet params:
    // Preamble=8, Header=explicit(0), PayloadLen=max(0xFF placeholder), CRC=on, IQ=standard
    uint8_t pkt[7] = { SX126X_SET_PACKET_PARAMS,
                       0x00, 0x08,     // preamble 8
                       0x00,           // explicit header
                       0xFF,           // payload len (we set per TX by writing buffer length effectively)
                       0x01,           // CRC on
                       0x00 };         // IQ standard
...
static void lora_send_frame(const uint8_t *d, size_t len)
{
    ...
    // write buffer: [opcode, offset=0x00, data...]
    uint8_t hdr[2] = { SX126X_WRITE_BUFFER, 0x00 };
    // send header
    sx126x_busy_wait();
    {
        ...
        t1.length = 16;
        t1.tx_buffer = hdr;
        spi_device_transmit(lora_spi, &t1);
    }
    // send payload
    {
        ...
        t2.length = len*8;
        t2.tx_buffer = d;
        spi_device_transmit(lora_spi, &t2);
    }
    sx126x_busy_wait();
    ...
    // We choose ~2s timeout => 2,000,000us / 15.625us = 128000 => 0x01F400
    uint32_t to = 128000;
...
static bool lora_try_read_frame(uint8_t *buf, size_t maxlen, uint8_t *out_len, int *out_rssi)
{
    ...
    // Read buffer:
    // 1) We need Rx buffer status (payload len + start pointer).
    // SX126x has GetRxBufferStatus (0x13). We'll implement minimal inline:
    // response: [payloadLen, startPointer]
    uint8_t tx[2] = { 0x13, 0x00 };
    uint8_t rx[4] = {0};
    sx126x_busy_wait();
    sx126x_xfer(tx, rx, 4);
    sx126x_busy_wait();
    ...
    // Packet status for RSSI (GetPacketStatus -> first byte is RSSI_sync in steps of -0.5dB)
    uint8_t st[3]={0};
...
static void batt_poll(void)
{
    ...
    if(g_adc_cali_ok){
        ...
        // crude fallback: scale raw to mV; not perfect, but usable
        // IDF typical 12-bit -> 4095. With 11dB atten max ~ 3.3V-ish at ADC input (varies).
        mv = (raw * 3300) / 4095;
    }

    // divider to battery mv
...
static esp_err_t api_status_get(httpd_req_t *req)
{
    ...
    // battery string (FIX: Typen passen zu PRIu32 / %u)
...
static void send_data_to(const char *dst_str, const char *txt, bool ackreq)
{
    ...
    // Für Robustheit in gemischten Starts: Broadcast fallback.
    call7_set(h->next_hop, "*");
...
void app_main(void)
{
    ESP_LOGI(TAG,"MeshRadio start (ESP-IDF 5.5.2) – SX1262/SX1276 switchable");
