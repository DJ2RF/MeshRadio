...
// ADC (Heltec V3 hat oft VBAT sense / ctrl – je nach Board. Hier: ADC GPIO34 als Beispiel)
...
#ifndef RELAY_GPIO
#define RELAY_GPIO       21     // default: bitte prüfen, ob bei dir frei/verdrahtet
#endif
...
#define SX126X_SET_RX_DUTY_CYCLE   0x94  // Rx duty cycle (sleep between RX windows)
...
// Heltec-typische opcodes (aus deinem funktionierenden Test)
...
{
    // format: opcode, 0x00(dummy), then read bytes
    uint8_t tx[2+64]={0};
    uint8_t rx[2+64]={0};
    if(n > 64) return;
...
{
    uint8_t cmd[3];
    cmd[0]=SX126X_CLEAR_IRQ_STATUS;
    cmd[1]=(uint8_t)(mask>>8);
    cmd[2]=(uint8_t)(mask&0xFF);
    sx126x_cmd(cmd, sizeof(cmd));
}

// Heltec/SX1262: PacketParams müssen die *echte* Payload-Länge enthalten,
// sonst ist TX/RX (inkl. IRQs) unzuverlässig. Wir halten die Parameter konstant
// und ändern nur payload_len.
static void sx126x_set_packet_params(uint8_t payload_len)
{
    // LoRa PacketParams:
    //  - PreambleLen: 8
    //  - HeaderType : Explicit (0x00)
...
// --- FIX #2: WriteBuffer (Opcode+Offset+Payload) MUSS in einer Transaktion ---
static void sx126x_cmd_with_payload(uint8_t opcode, const uint8_t *payload, size_t n)
{
    ...
    tx[0] = opcode;
    if(n) memcpy(&tx[1], payload, n);

    sx126x_busy_wait();
    sx126x_xfer(tx, rx, 1 + n);
    sx126x_busy_wait();
}
...
static void lora_init_radio(void)
{
    ...
    // Standby XOSC (0x01) – Heltec stabiler mit XOSC/TCXO
    { uint8_t stby[2] = { SX126X_SET_STANDBY, 0x01 }; sx126x_cmd(stby, sizeof(stby)); }

    // Heltec: TCXO on DIO3 (1.8V, delay 0x000064)
    ...
    // TxParams: power + ramp
    // power: 0x16 (~22 dBm on some configs) is not always allowed; choose a safe default 14 dBm (0x0E).
    // ramp: 0x04 (200us) common.
    { uint8_t txp[3] = { SX126X_SET_TX_PARAMS, 0x0E, 0x04 }; sx126x_cmd(txp, sizeof(txp)); }

    // RF frequency: DEFAULT_RF_FREQ_HZ
    // SX126x uses PLL steps: rf = (freq_hz << 25) / 32e6
    uint32_t rf = (uint32_t)(((uint64_t)DEFAULT_RF_FREQ_HZ << 25) / 32000000ULL);
    uint8_t rfcmd[5] = { SX126X_SET_RF_FREQUENCY,
                         (uint8_t)(rf>>24), (uint8_t)(rf>>16),
                         (uint8_t)(rf>>8),  (uint8_t)(rf) };
    sx126x_cmd(rfcmd, sizeof(rfcmd));

    // Modulation params: SF7, BW125kHz(0x04), CR4/5(0x01), LDRO=0
    uint8_t mod[5] = { SX126X_SET_MODULATION, 0x07, 0x04, 0x01, 0x00 };
    sx126x_cmd(mod, sizeof(mod));

    // Packet params: Preamble=8, Explicit header, PayloadLen set per TX (we still set here to 0xFF placeholder)
    uint8_t pkt[7] = { SX126X_SET_PACKET_PARAMS,
                       0x00, 0x08,
                       0x00,
                       0xFF,
                       0x01,
    ...
    uint8_t base[3] = { SX126X_SET_BUFFER_BASE, 0x00, 0x00 };
    sx126x_cmd(base, sizeof(base));

    // IRQ mapping: enable RX_DONE/TX_DONE/CRC_ERR/TIMEOUT to DIO1
    ...
    // RX continuous: SetRx(0xFFFFFF) => continuous
...
// RX duty-cycle: the radio sleeps internally between RX windows.
// This is the most "packet-safe" way for SX1262 because it keeps RX state inside the chip.
static void lora_set_rx_duty(uint32_t rx_on_ms, uint32_t rx_off_ms)
{
    // SX126x uses 15.625us steps (1/64000 s). For ms => steps = ms * 64
    uint32_t on  = rx_on_ms  * 64U;
    uint32_t off = rx_off_ms * 64U;

    lora_spi_lock();
    uint8_t cmd[7] = {
        SX126X_SET_RX_DUTY_CYCLE,
        (uint8_t)(on >> 16),  (uint8_t)(on >> 8),  (uint8_t)(on),
...
static void lora_send_frame(const uint8_t *d, size_t len)
{
    ...
    // FIX #2: WriteBuffer in ONE transaction: [0x0E][offset=0][payload...]
    uint8_t wb[1 + 255];
    wb[0] = 0x00; // offset
    memcpy(&wb[1], d, len);
    sx126x_cmd_with_payload(SX126X_WRITE_BUFFER, wb, 1 + len);

    // Set packet params with correct payload length
    uint8_t pkt[7] = { SX126X_SET_PACKET_PARAMS,
                       0x00, 0x08,
                       0x00,
                       (uint8_t)len,
    ...
    // SetTx(timeout) -> timeout in units of 15.625us
    uint32_t to = 128000; // ~2s
...
static bool lora_try_read_frame(uint8_t *buf, size_t maxlen, uint8_t *out_len, int *out_rssi)
{
    ...
    // Clear RX_DONE early to avoid retrigger storms
    sx126x_clear_irq(SX126X_IRQ_RX_DONE);

    // FIX #3: GetRxBufferStatus (0x13) correct clocking: opcode + dummy + 2 bytes
    uint8_t tx[4] = { 0x13, 0x00, 0x00, 0x00 };
    uint8_t rx[4] = { 0 };
    sx126x_busy_wait();
    sx126x_xfer(tx, rx, 4);
    sx126x_busy_wait();
    ...
    // ReadBuffer(op, offset, dummy, data...)
    uint8_t tx2[3] = { SX126X_READ_BUFFER, start, 0x00 };
    uint8_t rx2[3+255] = {0};
    sx126x_busy_wait();
    sx126x_xfer(tx2, rx2, 3+payLen);
    sx126x_busy_wait();
    memcpy(buf, rx2+3, payLen);

    // Packet status for RSSI (GetPacketStatus -> RSSI_sync in steps of -0.5dB)
    uint8_t st[3]={0};
...
static void lora_log_chip_info(void)
{
    ESP_LOGI(TAG,"LoRa chip: SX1262 (Heltec V3.x preset)");
}
...
// ============================================================================
//
// IMPORTANT REALITY CHECK (LoRa + Sleep):
// - If the radio (or the ESP32) is asleep, you *can* miss packets.
// - The only way to be 100% lossless is: "always-on RX" (RELAY mode).
// - For EDGE/SENSOR we instead use duty-cycled RX windows. Your protocol must
//   retransmit / beacon / ack-timeout accordingly.
//
// This code adds:
//   - SENSOR: periodic wake, short RX window, then deep-sleep.
//   - EDGE  : optional duty-cycled RX windows (light-sleep between windows).
//
// Defaults are controlled by:
//   MR_POWERSAVE_ENABLE, DEFAULT_NODE_MODE, EDGE_DUTY_RX_ENABLE, EDGE_RX_ON_MS,
//   EDGE_RX_OFF_MS, SENSOR_WAKE_PERIOD_MS, SENSOR_RX_WINDOW_MS.
//

#if MR_POWERSAVE_ENABLE

static void lora_sleep_radio(void)
{
#if defined(MR_LORA_CHIP_SX1276)
    // SX1276: Sleep mode (LoRa)
    lora_spi_lock();
    sx1276_wr(SX1276_REG_OP_MODE, 0x80);
    lora_spi_unlock();
#elif defined(MR_LORA_CHIP_SX1262)
    // SX1262: SetSleep (0x84). Param 0x00 = warm-start, keep config.
    lora_spi_lock();
    uint8_t slp[2] = { 0x84, 0x00 };
    sx126x_cmd(slp, sizeof(slp));
    lora_spi_unlock();
#endif
}

static void lora_wake_radio(void)
{
    // After sleep, just go back to continuous RX.
    lora_set_rx_continuous();
}

static void powersave_config_wakeup(void)
{
    // Timer wake is used for EDGE off-time and SENSOR periodic wake.
    // IRQ wake allows the ESP32 to wake as soon as a packet arrives.
#if defined(MR_LORA_CHIP_SX1276)
    const gpio_num_t irq = (gpio_num_t)PIN_LORA_DIO0;
#elif defined(MR_LORA_CHIP_SX1262)
    const gpio_num_t irq = (gpio_num_t)PIN_LORA_DIO1;
#else
    const gpio_num_t irq = (gpio_num_t)-1;
#endif
...
static void powersave_edge_loop(void)
{
#if EDGE_DUTY_RX_ENABLE
    // EDGE: RX for EDGE_RX_ON_MS, then light-sleep for EDGE_RX_OFF_MS.
    while(g_node_mode == NODE_EDGE){
        // ensure RX is running
#if defined(MR_LORA_CHIP_SX1262)
        // prefer internal duty-cycling (radio handles sleep between windows)
        lora_set_rx_duty(EDGE_RX_ON_MS, EDGE_RX_OFF_MS);

        // Let the CPU sleep; it will wake on IRQ or timer.
        esp_sleep_enable_timer_wakeup((uint64_t)(EDGE_RX_ON_MS + EDGE_RX_OFF_MS) * 1000ULL);
        esp_light_sleep_start();

        // After wake: process pending RX events quickly
        uint32_t t0 = now_ms();
        while(now_ms() - t0 < EDGE_RX_ON_MS){
            handle_rx();
            vTaskDelay(pdMS_TO_TICKS(10));
        }
#else
        // SX1276: no RxDutyCycle command -> emulate with light-sleep
        lora_set_rx_continuous();
        uint32_t t0 = now_ms();
        while(now_ms() - t0 < EDGE_RX_ON_MS){
            handle_rx();
        ...
        lora_sleep_radio();
        esp_sleep_enable_timer_wakeup((uint64_t)EDGE_RX_OFF_MS * 1000ULL);
        esp_light_sleep_start();
        lora_wake_radio();
#endif
    }
...
static void powersave_sensor_once_then_sleep(void)
{
    // SENSOR: short RX window, then deep-sleep until next wake period.
    // NOTE: deep-sleep resets the CPU, so your program restarts in app_main().
    lora_set_rx_continuous();

    uint32_t t0 = now_ms();
    while(now_ms() - t0 < SENSOR_RX_WINDOW_MS){
        handle_rx();
        vTaskDelay(pdMS_TO_TICKS(10));
    }
    ...
    // Wake again after SENSOR_WAKE_PERIOD_MS
    esp_sleep_enable_timer_wakeup((uint64_t)SENSOR_WAKE_PERIOD_MS * 1000ULL);
    ESP_LOGI(TAG, "SENSOR: deep-sleep for %u ms", (unsigned)SENSOR_WAKE_PERIOD_MS);
...
{
    // ADC1 mapping for common pins; we support typical ones.
    switch(gpio){
...
{
#if (MR_BOARD_PRESET == MR_BOARD_HELTEC_V3)
    // Heltec V3: Battery divider must be enabled (GPIO37 LOW) to connect VBAT to ADC.
    // Keep it OFF by default to reduce drain; batt_poll() will toggle it for each sample.
    ...
    // Map GPIO -> ADC unit/channel (works on ESP32 + ESP32-S3)
    adc_unit_t unit = ADC_UNIT_1;
    adc_channel_t ch = 0;
    esp_err_t e = adc_oneshot_io_to_channel((gpio_num_t)BATT_ADC_GPIO, &unit, &ch);
    if(e != ESP_OK){
        ESP_LOGW(TAG, "VBAT GPIO%d -> adc_oneshot_io_to_channel failed: %s", BATT_ADC_GPIO, esp_err_to_name(e));
        return;
    }
    ESP_LOGI(TAG, "VBAT GPIO%d -> %s unit=%d ch=%d", BATT_ADC_GPIO, esp_err_to_name(e), (int)unit, (int)ch);
    ...
    adc_oneshot_chan_cfg_t chan_cfg = {
        .atten = ADC_ATTEN_DB_12,              // full range (ADC_ATTEN_DB_11 is deprecated alias)
        .bitwidth = ADC_BITWIDTH_DEFAULT
    };
    ESP_ERROR_CHECK(adc_oneshot_config_channel(g_adc_unit, ch, &chan_cfg));
...
#else
    ESP_LOGI(TAG, "ADC cali not enabled in this ESP-IDF build -> using raw fallback");
#endif
...
static void batt_poll(void)
{
    ...
    int raw=0;
    if(adc_oneshot_read(g_adc_unit, (adc_channel_t)ch, &raw) != ESP_OK) return;

    int mv=0;
    if(g_adc_cali_ok){
        if(adc_cali_raw_to_voltage(g_adc_cali, raw, &mv) != ESP_OK) mv=0;
    }else{
        mv = (raw * 3300) / 4095;
    }

    float scale = (BATT_DIV_RTOP_OHMS + BATT_DIV_RBOT_OHMS) / BATT_DIV_RBOT_OHMS;
    uint32_t vbat = (uint32_t)((float)mv * scale);
...
#if (MR_BOARD_PRESET == MR_BOARD_HELTEC_V3)
    // FIX #1: Heltec V3.x benötigt VEXT (GPIO36) LOW => ON
    gpio_set_direction((gpio_num_t)PIN_VEXT_CTRL, GPIO_MODE_OUTPUT);
    gpio_set_level((gpio_num_t)PIN_VEXT_CTRL, 0);
    vTaskDelay(pdMS_TO_TICKS(100));
#endif
...
#if MR_POWERSAVE_ENABLE
        // SENSOR: do RX window, then deep-sleep (never returns)
        if(g_node_mode == NODE_SENSOR){
            powersave_sensor_once_then_sleep();
        }
        // EDGE: optional duty-cycle loop (returns immediately if disabled)
        if(g_node_mode == NODE_EDGE){
            powersave_edge_loop();
        }
#endif
#if MR_BATT_ENABLE
