 ...
 *   - Kürzer = schnelleres Discovery, mehr Airtime
 *   - Länger = weniger Airtime, trägere Discovery
 *
 * BEACON_JITTER_MS:
 ...
 *   - Hilft bei Kollisionen/Hidden Nodes
 */
 ...
 *   - Dient als Deckel gegen unendlich schlechte Links
 *
 * ETX_DECAY_MS:
 *   - Halbwerts-Decay für Link-Statistik (tx_attempts/ack_ok)
 *   - Verhindert, dass alte Werte ewig nachwirken
 */
 ...
 *   - 1: Route-Advertisements aktiv (Node teilt Top-Routen)
 *   - 0: Nur Beacons / kein Routing-Advertise
 *
 * DEFAULT_ROUTEADV_TOPN:
 *   - Anzahl Top-Routen, die in Route-Advertisements gesendet werden
 *
 * DEFAULT_ROUTEADV_DELTA_ETX:
 *   - Hysterese/Schwellwert für Route-Update (ETX-Vergleich)
 *   - Höher = stabiler, aber langsameres Umschalten
 *
 * DEFAULT_HOLDDOWN_MS:
 *   - Hold-Down Zeit nach Next-Hop Wechsel (Route-Flapping vermeiden)
 */
...
/*
 * Token-Bucket:
 *   - Schützt Funkkanal vor Flooding
 *   - tps   = Tokens pro Sekunde (Durchsatz)
 *   - burst = Max. Burst (Kurzzeit-Spitzen)
 *
 * RL_BEACON_*:
 *   - Begrenzung für Beacons
 *
 * RL_ROUTEADV_*:
 *   - Begrenzung für Route-Advertisements
 *
 * RL_ACK_*:
 *   - Begrenzung für ACK Frames
 *
 * RL_DATA_*:
 *   - Begrenzung für DATA Frames (Nutzdaten)
 */
...
/*
 * DEFAULT_CAD_ENABLE:
 *   - 1: CAD (Channel Activity Detection) / extra Backoff vor TX
 *   - 0: Aus (simpler, meist ausreichend)
 *
 * CAD_WAIT_MS:
 *   - Basis-Wartezeit vor TX wenn CAD enabled
 *
 * CAD_JITTER_MS:
 *   - Zufallsanteil (0..CAD_JITTER_MS) zusätzlich zur Basiswartezeit
 */
 ...
 *   - 1: AES-CCM Verschlüsselung/Authentifizierung aktiv (MR_FLAG_SEC)
 *   - 0: Klartext (Debug/Kompatibilität)
 *
 * MR_NET_KEY_HEX:
 *   - 16-Byte Netz-Schlüssel als 32 Hex-Zeichen
 *   - Muss auf allen Nodes identisch sein, sonst keine Entschlüsselung möglich
 *
 * MR_NET_ID:
 *   - Netzkennung (1 Byte) als zusätzlicher Nonce-Bestandteil
 *   - Trennt Netze logisch (zusammen mit Key)
 */
...
/*
 * SEC_KEY_LEN:
 *   - AES Key Länge in Bytes (16 = AES-128)
 *
 * SEC_NONCE_LEN:
 *   - CCM Nonce Länge (hier 12 Byte)
 *
 * SEC_TAG_LEN:
 *   - Auth-Tag Länge in Bytes (hier 8 Byte)
 *   - Kürzer = weniger Overhead, aber geringere Sicherheitsreserve
 */
...
/*
 * DEFAULT_NODE_MODE:
 *   - 0 = RELAY  : Router/Forwarder (Dauerbetrieb)
 *   - 1 = EDGE   : Endpoint (optional Duty-RX)
 *   - 2 = SENSOR : Sensor/Low-Power (optional DeepSleep)
 */
 ...
 *   - 0: Dauerbetrieb (RELAY/EDGE normalerweise)
 *
 * SENSOR_WAKE_PERIOD_MS:
 *   - Schlafdauer zwischen Wakeups (SENSOR)
 *
 * SENSOR_BOOT_RX_WINDOW_MS:
 *   - RX-Fenster direkt nach Boot/Wakeup, um Downlink-Kommandos zu empfangen
 */
...
/*
 * EDGE_DUTY_RX_ENABLE:
 *   - 1: EDGE Duty-Cycle RX (periodisch RX an/aus)
 *   - 0: EDGE verhält sich wie normaler Node (RX kontinuierlich)
 *
 * EDGE_RX_ON_MS / EDGE_RX_OFF_MS:
 *   - Duty Cycle Parameter (RX an / RX aus)
 *
 * MR_RELAY_CALLSIGN:
 *   - Ziel-Callsign des Relays, an das SENSOR/EDGE Status/Telemetrie senden können
 */
 ...
 *   - 1: WiFi + HTTP Dashboard direkt beim Boot starten
 *   - 0: WiFi aus (kann per CLI/Web-API später aktiviert werden)
 */
...
/*
 * MR_RELAY_ENABLE:
 *   - 1: Relay GPIO Feature aktiv (z.B. externes Relais/Power-Switch)
 *   - 0: Relay Feature deaktiviert (Code kompakter)
 *
 * RELAY_ACTIVE_LEVEL:
 *   - 1: GPIO HIGH = Relais ON
 *   - 0: GPIO LOW  = Relais ON (Active-Low Boards)
 */
 ...
 *   - 0: Battery Code aus
 *
 * BATT_MEASURE_INTERVAL_MS:
 *   - Messintervall in ms
 *
 * BATT_EMPTY_MV / BATT_FULL_MV:
 *   - Mapping von mV auf Prozent (linear)
 *   - Voll/Leer-Schwellen an Akku/Board anpassen
 *
 * MR_ADC_CALI_MODE:
 *   - 1: ADC Kalibrierung aktiv (wenn verfügbar)
 *   - 0: Roh/Approximation (weniger genau, aber simpel)
 */
...
/*
 * MR_CLI_ENABLE:
 *   - 1: UART CLI aktiv (Debug/Steuerung)
 *   - 0: CLI aus (weniger Code/Tasks)
 *
 * MR_CLI_LINE_MAX:
 *   - Maximale Eingabezeile (Puffergröße)
 */
...
/*
 * MR_BME280_ENABLE:
 *   - 1: BME280 Sensor aktiv (I2C Wetterdaten)
 *   - 0: Sensor-Code deaktiviert
 *
 * BME280_I2C_ADDR:
 *   - I2C Adresse des BME280 (typisch 0x76 oder 0x77)
 *
 * BME280_I2C_CLK_HZ:
 *   - I2C Clock (100kHz = robust, 400kHz = schneller aber empfindlicher)
 */
...
/*
 * BME_PRESS_OFFSET_HPA:
 *   - Druck-Offset in hPa (Kalibrier-/Korrekturwert)
 *   - Hilft, wenn dein Sensor systematisch zu niedrig/hoch misst
 */
#define BME_PRESS_OFFSET_HPA  50
...
// I2C am externen Header (Heltec V3.x/V3.2)
#define PIN_I2C_SDA      41
#define PIN_I2C_SCL      42

// RELAY: WICHTIG: darf NICHT 14 sein (IRQ)!
#ifndef RELAY_GPIO
#define RELAY_GPIO       35  // default: bitte prüfen, ob bei dir frei/verdrahtet
#endif
...
// ✅ Battery sense beim T3 V1.6.1: GPIO35, Teiler ~2:1
...
void app_main(void)
{
    ...
    while(1){
        ...
        // Falls du später EDGE Duty-RX einbauen willst, wäre hier der Platz.
    }
}