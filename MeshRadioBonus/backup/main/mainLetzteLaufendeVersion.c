...
#define MR_POWERSAVE_ENABLE       1
#define SENSOR_WAKE_PERIOD_MS     30000
#define SENSOR_RX_WINDOW_MS       30000
...
#ifndef RELAY_GPIO
#define RELAY_GPIO       35 // 4     // default: bitte prüfen, ob bei dir frei/verdrahtet
#endif
...
// ============================================================================
#if MR_RELAY_ENABLE
// global irgendwo (oben im File):

static void relay_init(void)
{
    bool on = (rtc_relay_state != 0);
    int level_on  = (RELAY_ACTIVE_LEVEL ? 1 : 0);
    int level_off = (RELAY_ACTIVE_LEVEL ? 0 : 1);
    int want = on ? level_on : level_off;
    ...
    // WICHTIG: falls Hold aktiv ist: nur diesen Pin freigeben,
    // dann SOFORT den gewünschten Pegel setzen
    gpio_hold_dis((gpio_num_t)RELAY_GPIO);
    gpio_set_level(RELAY_GPIO, want);
...
// Optional: Boot-RX-Fenster für SENSOR (ms)
#ifndef SENSOR_BOOT_RX_WINDOW_MS
...
void app_main(void)
{
    ...
    // =========================================================================
    // 0) WAKEUP-HOUSEKEEPING (GANZ AM ANFANG!)
    //    - Wenn du vorher gpio_hold_en() + gpio_deep_sleep_hold_en() genutzt hast,
    //      sind Pins nach dem Wake "gelatcht".
    //    - Hold MUSS VOR JEDEM gpio_set_level()/gpio_config()/relay_init() gelöst werden,
    //      sonst wirkt relay_init() ggf. nicht oder der Pin bleibt "fest".
    // =========================================================================
    
    //gpio_deep_sleep_hold_dis();  // global hold off (nach Wake)

/*#if MR_RELAY_ENABLE
    gpio_hold_dis((gpio_num_t)RELAY_GPIO);   // Relay-Pin freigeben (falls gelatcht)
#endif
*/
#if (MR_BOARD_PRESET == MR_BOARD_HELTEC_V3)
    // Heltec VEXT: auch freigeben, falls gelatcht
    ...
    // Wichtig: msg_id/seq stabil über DeepSleep
    // (sonst Dupe-Filter killt wiederholte "AWAKE")
    ...
    //    WICHTIG:
    //      - relay_init() erst NACH Hold-Release (oben), sonst kann es "nicht wirken".
    //      - relay_init() sollte idealerweise den RTC-Relaiszustand respektieren
    //        (also NICHT stumpf default OFF), sonst fällt es beim Wake kurz ab.
    // =========================================================================
#if MR_RELAY_ENABLE
    relay_init();
#endif
...
#if MR_BATT_ENABLE
    batt_init();
    // Force a first reading immediately (important for SENSOR which may sleep early)
    xSemaphoreTake(g_mutex, portMAX_DELAY);
    g_next_batt_ms = 0;   // allow batt_poll() to run right now
    xSemaphoreGive(g_mutex);

    batt_poll();          // do one measurement on boot
    #if (MR_BOARD_PRESET == MR_BOARD_HELTEC_V3)
        batt_path_enable(false); // divider OFF after init
    #endif
#endif
    ...
    //
    //    Ablauf:
    //      - Radio RX continuous sicherstellen
    //      - "SENSOR:AWAKE" an Relay schicken (ACKREQ=true => retry_task sorgt für Wiederholung)
    //      - RX-Fenster offen halten => Relay kann Kommandos schicken
    //      - Wenn MR_POWERSAVE_ENABLE: GPIOs halten + DeepSleep
    // =========================================================================
    if(g_node_mode == NODE_SENSOR){

        // Sicherstellen, dass wir RX sind
        lora_set_rx_continuous();

        // Kleines Stabilitäts-Delay nach Radio-Init (hilft oft bei Heltec/SX1262)
        vTaskDelay(pdMS_TO_TICKS(150));

        // "Ich bin wach" – mit ACKREQ, damit zuverlässig.
        // Voraussetzung: send_data_to() muss pending_add_locked() + neighbor_tx_attempt_locked()
        // beim ackreq enthalten (dein Patch).
        send_data_to(MR_RELAY_CALLSIGN, "SENSOR:AWAKE", true);

        // Downlink-Fenster: RELAY kann CMDs schicken, Sensor empfängt/ack't
        sensor_awake_window(SENSOR_BOOT_RX_WINDOW_MS);

#if MR_POWERSAVE_ENABLE
        // -------------------------------------------------------------
        // Vor DeepSleep: Relais-/LED-Pin + (Heltec) VEXT halten
        // -------------------------------------------------------------
    #if MR_RELAY_ENABLE
        // Relay-Pin latched halten
        gpio_hold_en((gpio_num_t)RELAY_GPIO);
    #endif

    #if (MR_BOARD_PRESET == MR_BOARD_HELTEC_V3)
        // WICHTIG: Wenn dein Relais/LED/Peripherie an VEXT hängt,
        // muss VEXT ebenfalls gehalten werden, sonst fällt es ab obwohl GPIO gehalten wird.
        gpio_set_direction((gpio_num_t)PIN_VEXT_CTRL, GPIO_MODE_OUTPUT);
        gpio_set_level((gpio_num_t)PIN_VEXT_CTRL, 0);    // VEXT ON (LOW)
        //gpio_hold_en((gpio_num_t)PIN_VEXT_CTRL);
    #endif

        // Global DeepSleep Hold aktivieren (digital pins halten)
        gpio_deep_sleep_hold_en();
        ...
        // DeepSleep sollte nicht zurückkehren – falls doch, hier parken.
        while(1){ vTaskDelay(pdMS_TO_TICKS(1000)); }

#else
        // Ohne PowerSave bleibt SENSOR dauerhaft im Loop (wie ein normaler Node),
        // sendet aber keine Beacons (siehe Loop unten).
    ...
    while(1){
        ...
        // EDGE Duty-RX wäre hier später der Platz.
    }
}