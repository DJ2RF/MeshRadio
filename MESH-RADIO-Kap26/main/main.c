// ============================================================================
// MeshRadio – Kapitel 26 (Loop-sicheres Routing) – KOMPLETTE SOURCEN (wie 25)
// ----------------------------------------------------------------------------
// Kapitel 26 Ziel:
//   - Routing-Loops vermeiden, ohne viel Overhead
//
// Neue Konzepte:
//   - Route Sequence Number (seq) im BEACON (und optional in DATA)
//   - Update-Regeln:
//       * new.seq > old.seq                    -> übernehmen
//       * new.seq == old.seq UND bessere metric -> übernehmen
//       * sonst ignorieren
//   - Feasibility Rule (Light):
//       * Niemals Route übernehmen, wenn next == ME
//
// Features (wie Kapitel 25 Web-UI):
//   - Web UI: Startseite "/" (kein 404 mehr)
//   - /api/send        (dst,msg)
//   - /api/routes      (dst -> next (metric, seq, age_ms))
//   - /api/bcfallback  (enable=0/1)
//
// Hinweis:
//   - Dieses Kapitel fokussiert Routing-Stabilität. Kein ACK/Store hier.
//   - Metric ist hier noch RSSI-basiert (Kapitel 27 ersetzt das durch ETX-Light).
// ============================================================================
...
// Beacon / Routing
#define BEACON_INTERVAL_MS 15000               // Beacon alle X ms (+ Jitter)
...
// PROTOKOLL
// ============================================================================
#define MR_PROTO_VERSION 4        // neue Version wegen seq
...
typedef struct {
    ...
    // Kapitel 26: Route Sequence Number
    uint16_t seq;
...
// Kapitel 26: Route enthält seq + metric
typedef struct {
    bool used;
    char dst[7];
    char next[7];
    int metric;        // Kapitel 26: RSSI als simple metric
    uint16_t seq;      // Route-Version
    uint32_t t_ms;
} route_t;
...
// ============================================================================
// ROUTES – Kapitel 26 Kern (seq + Update-Regeln + Feasibility Light)
// ============================================================================
static void route_update_locked(const char dst[7],
                                ...
                                uint16_t seq)
{
    // Feasibility Rule (Light): niemals Route übernehmen, die über uns selbst führt
    char me[7]; call7_set(me, MR_CALLSIGN);
    if(call7_eq(next, me)){
        // Loop-Bremse: ignorieren
        return;
    }
    ...
    for(int i=0;i<MAX_ROUTES;i++){
        ...
        if(seq > routes[i].seq){
            replace=true;
        }else if(seq == routes[i].seq && metric > routes[i].metric){
            replace=true;
        }
...
{
    if(len < sizeof(mr_hdr_v4_t)) return;
    mr_hdr_v4_t *h=(mr_hdr_v4_t*)buf;
...
static void handle_rx(void)
{
    ...
    if(h->flags & MR_FLAG_BEACON){
        ESP_LOGI(TAG,"RX BEACON src=%s via=%s rssi=%d seq=%u", srcs, lhs, rssi, h->seq);

        // Kapitel 26: Route Learning mit seq + Update-Regeln
        xSemaphoreTake(g_mutex,portMAX_DELAY);
        route_update_locked(h->src, h->last_hop, rssi, h->seq);
        xSemaphoreGive(g_mutex);
        return;
    }
    ...
    if(h->flags & MR_FLAG_DATA){
        ...
        if(h->payload_len>0 && h->payload_len<=MAX_PAYLOAD){
            memcpy(txt, buf+sizeof(mr_hdr_v4_t), h->payload_len);
            txt[h->payload_len]=0;
        }
...
// ============================================================================
// HTTP UI + API – wie Kapitel 25
// ============================================================================
static const char *INDEX_HTML =
"<!doctype html><html><head><meta charset='utf-8'>"
"<meta name='viewport' content='width=device-width,initial-scale=1'>"
"<title>MeshRadio Kapitel 26</title>"
"<style>"
...
"</style></head><body>"
"<h2>MeshRadio Kapitel 26 – Loop-sicheres Routing (seq)</h2>"
"<p>AP: <code>" MR_WIFI_AP_SSID "</code>  URL: <code>http://192.168.4.1</code></p>"
"<div id='m'></div>"
"<h3>DATA senden</h3>"
"<label>Ziel (dst)</label><input id='dst' value='DJ2RF'>"
"<label>Text</label><input id='msg' value='Hello 26'>"
...
static esp_err_t api_routes_get(httpd_req_t *req)
{
    ...
    for(int i=0;i<MAX_ROUTES;i++){
        ...
        off += snprintf(out+off,sizeof(out)-off,
                        "%s -> %s  (m=%d seq=%u age=%" PRIu32 ")\n",
                        d,n,routes[i].metric,(unsigned)routes[i].seq,age);
...
void app_main(void)
{
    ESP_LOGI(TAG,"MeshRadio Kapitel 26 start (Loop-safe routing + WebUI)");
