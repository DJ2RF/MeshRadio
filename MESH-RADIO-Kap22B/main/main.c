...
// ----------------------------------------------------------------------------
// KONFIGURATION WIE 22A – ABER PER WLAN (Webserver) STATT UART-CLI
//
// Basis (aus Kap. 22A übernommen):
//   ✔ NVS Persistenz: call, freq_hz, sf, bw_khz, crc_on
//   ✔ SX1276: SF/BW/CRC korrekt in Register (REG_MODEM_CONFIG_1/2/3)
//   ✔ Beacon TX/RX (Route Refresh) mit Header V3 (last_hop)
//
// NEU (Kap. 22B):
//   ✔ WiFi AP "MeshRadio-Setup" (IP: 192.168.4.1)
//   ✔ Webserver (esp_http_server):
//        GET  /            -> Setup-Webseite (HTML)
//        GET  /api/show    -> JSON Config
//        POST /api/set     -> Config im RAM setzen (noch nicht apply)
//        POST /api/apply   -> LoRa apply (wie Kap 22A)
//        POST /api/save    -> NVS speichern
//        POST /api/load    -> NVS laden + apply
//        POST /api/reboot  -> Neustart
//
// Hinweise:
//   - AP-Modus ist immer erreichbar, kein Router notwendig.
//   - Für das Buch ideal: Browser auf, Werte ändern, Apply/Save klicken.
//
// ESP-IDF v5.5.x
// ============================================================================
...
// ============================================================================
// LORA PINS (TTGO T-Beam V1.1) – wir bleiben hier bewusst bei SX1276 Boards
// ============================================================================
...
// Beacon runtime params (kann später auch per Web gemacht werden)
...
static void wifi_init_ap(void)
{
    ...
    // Offenes AP (ohne Passwort) – für Setup bequem
    // (Wenn du Passwort willst: WPA2 + passphrase eintragen)
    wifi_config_t ap = {0};
...
{
    // body is modifiable (we can insert '\0')
...
"}"
"async function doSet(){ await post('/api/set'); }"
"async function doApply(){ await post('/api/apply'); }"
"async function doSave(){ await post('/api/save'); }"
...
static esp_err_t http_api_show(httpd_req_t *req)
{
    ...
    snprintf(json, sizeof(json),
             "{\"call\":\"%s\",\"freq\":%" PRIu32 ",\"sf\":%u,\"bw\":%u,\"crc\":%u}",
             g_cfg.call, g_cfg.freq_hz, g_cfg.sf, g_cfg.bw_khz, g_cfg.crc_on);
    xSemaphoreGive(g_cfg_mutex);
...
void app_main(void)
{
    ESP_LOGI(TAG,"MeshRadio Kapitel 22B start (WiFi Web-Setup)");
